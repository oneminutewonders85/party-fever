  ...
  if (se.phase === 'finished') {
    ...
    return (
      <div className="flex min-h-[86vh] flex-col items-center justify-center text-center">
        <div className="absolute left-10 top-8"><ExitButton label="Back to games" roomId={room.id} /></div>
        ...
        <div className="flex w-[380px] flex-col gap-6">
          {se.phase === 'play' ? (
            <div className="pf-glass rounded-card p-6 text-center">
              <p className={`mb-3 text-sm uppercase tracking-widest ${arm.holding ? 'text-amber-300' : 'text-white/50'}`}>
            {arm.holding ? 'Get ready…' : 'GO! Find objects starting with'}
          </p>
