...
import { useWw, wwGetMe, wwSubmitAction, wwSubmitVote } from './api'
import { PlayerPicker, ROLE_ACCENT, ROLE_LABEL, RoleIcon } from './ui'
import { GhostIcon, ShieldEyeIcon } from '../../shared/components/icons'
...
export default function WerewolfPhone({ room, me, players }: PhoneViewProps) {
  ...
  if (!alive && ww.phase !== 'village_win' && ww.phase !== 'werewolf_win') {
    return (
      <Center>
        <p className="flex justify-center opacity-40"><GhostIcon size={56} /></p>
  ...
  if (ww.phase === 'role_reveal') {
    ...
    return (
      <div className="flex min-h-full flex-1 flex-col items-center justify-center px-6 text-center"
        ...
        <p className="flex justify-center" style={{ color: accent }}><RoleIcon role={role.role} size={72} /></p>
        ...
        <p className="mt-6 inline-flex items-center gap-2 text-sm text-p-lime"><ShieldEyeIcon size={18} /> Shield your screen</p>
  ...
  if (ww.phase === 'night') {
    ...
    async function submit() {
      if (!pick) return
      try { await wwSubmitAction(room.id, cfg.kind, pick); setLocked(true); syncCb.current(); chan.current?.sendSync() }
    ...
    return (
      <div className="flex min-h-full flex-1 flex-col px-5 py-6">
        <div className="mb-2 flex items-center justify-between">
          <p className="pf-wordmark inline-flex items-center gap-2 text-xl" style={{ color: accent }}><RoleIcon role={role.role} size={22} /> {ROLE_LABEL[role.role]}</p>
  ...
  if (ww.phase === 'discussion') {
    ...
    async function vote() {
      if (!pick) return
      try { await wwSubmitVote(room.id, pick); sound.init(); setLocked(true); syncCb.current(); chan.current?.sendSync() }
      catch (e) { console.error(e) }
    }
    return (
      <div className="flex min-h-full flex-1 flex-col px-5 py-6">
  ...
  return (
    <Center>
      ...
      <p className="mt-2 inline-flex items-center justify-center gap-2 text-white/70">You were the {ROLE_LABEL[role.role]} <RoleIcon role={role.role} size={20} /></p>
