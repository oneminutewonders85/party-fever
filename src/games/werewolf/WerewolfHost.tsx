...
import { NightScene, PlayerDot, ROLE_ACCENT, ROLE_ICON, ROLE_LABEL } from './ui'
...
export default function WerewolfHost({ room, players }: HostViewProps) {
  ...
  const remaining = useServerCountdown(ww?.phase_started, ww?.phase_len ?? 0)
        ...
        <NightScene>
          <div className="text-center">
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-3">
              {players.map((p) => (
                <span key={p.id} className="pf-glass grid h-14 w-14 place-items-center rounded-card text-2xl">🌙</span>
              ))}
            </div>
          </div>
        </NightScene>
  ...
  if (ww.phase === 'dawn') {
    ...
    return (
      <div className="mx-auto max-w-4xl text-center">
        ...
        <p className="mt-10 pf-wordmark text-2xl text-p-lime">Now — find the werewolf 🔍</p>
      </div>
    )
  }
          ...
          {players.map((p) => {
            ...
            return (
              <div key={p.id} className="pf-glass flex items-center gap-3 rounded-card px-4 py-2">
                ...
                {role && <span style={{ color: ROLE_ACCENT[role] }}>{ROLE_ICON[role]} {ROLE_LABEL[role]}</span>}
              </div>
            )
          })}
        </div>
