...
export default function SpinSpellHost({ room, players }: HostViewProps) {
  ...
  const spinHolding = useSpinHold(ss?.last)
            ...
            <div className="text-center">
              {ss.phase === 'reveal' ? (
                ...
                <>
                  {spinHolding ? (
                    <>
                      <p className="text-sm uppercase tracking-widest text-white/50">Round {ss.round}</p>
                      <p className="pf-wordmark mt-1 animate-pulse text-5xl text-amber-300">Spinning…</p>
                    </>
                  ) : (
                    <>
