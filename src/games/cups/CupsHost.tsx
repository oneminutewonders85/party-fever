...
export default function CupsHost({ room, players }: HostViewProps) {
  ...
  const roundImgs = cu?.theme && cu?.target ? cu.target.map((c) => itemSrc(cu.theme, c)) : []
  ...
  return (
    <div className="flex min-h-[86vh] flex-col">
      ...
      <div className="flex flex-1 flex-col items-center justify-center overflow-hidden text-center">
        ...
        <p className={`mb-4 pf-wordmark text-3xl lg:text-4xl ${arm.holding ? 'text-amber-300' : arm.justArmed ? 'text-p-lime' : 'text-white'}`}>
          {arm.holding ? 'Get ready…' : arm.justArmed ? 'START!' : 'Match it on your phone, then BUZZ!'}
        </p>
