...
export default function CupsPhone({ room, me }: PhoneViewProps) {
  // This view used to have no realtime channel at all: it neither heard the
  // host nor told it anything, so every buzz reached the TV only on the next
  // safety poll.
  ...
  async function buzz() {
    ...
    try {
      ...
      chan.current?.sendSync() // the TV decides the winner; tell it the moment we buzz
  ...
  return (
    <div className="flex min-h-full flex-1 flex-col px-4 py-4" style={{ touchAction: 'none' }}>
      ...
      <p className="mb-2 text-center text-sm text-white/60">
        {arm.holding
          ? <b className="text-amber-300">{arm.loading ? 'Loading…' : 'Get ready…'}</b>
      ...
      <div className="flex flex-1 items-center justify-center">
        <div
          ref={box}
          className="relative rounded-3xl border border-white/10 bg-white/5 transition-opacity"
          style={{
            width: 'min(92vw, 56vh)',
            aspectRatio: '1 / 1',
            opacity: arm.holding ? 0.45 : 1,
            // belt and braces: the drag handler already checks arm.armed, but
            // this makes the board visibly and physically inert while locked
            pointerEvents: arm.holding ? 'none' : 'auto',
          }}
        >
          {arm.justArmed && (
            <div className="pointer-events-none absolute inset-0 z-20 grid place-items-center">
              <span className="pf-wordmark animate-popIn text-6xl text-p-lime drop-shadow-lg">START!</span>
            </div>
          )}
      ...
      <button
        onClick={buzz}
        disabled={cooldown > 0 || arm.holding}
        ...
        style={{ background: cooldown > 0 || arm.holding ? '#9ca3af' : '#f4c20d' }}
      >
        {arm.holding ? 'Get ready…' : cooldown > 0 ? `Wait ${cooldown}s` : 'BUZZ!'}
      </button>
    </div>
  )
}
