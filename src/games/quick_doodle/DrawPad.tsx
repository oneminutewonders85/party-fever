...
export function DrawPad({
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      if (cur.current && pending.current.length) {
        channel.sendStroke({ id: cur.current.id, color, size: brush, points: pending.current })
        pending.current = []
      }
    }, 70)
    return () => window.clearInterval(id)
  }, [enabled, channel, color, brush])
  ...
  function down(e: React.PointerEvent) {
    ...
    const p = ptFrom(e)
    cur.current = { id: uid(), color, size: brush, points: [p] }
    pending.current = [p]
        ...
        <div className="flex items-center gap-3">
          {BRUSHES.map((b) => (
            <button
              ...
              className={`grid h-10 w-10 place-items-center rounded-full pf-glass ${brush === b ? 'ring-2 ring-cyan' : ''}`}
            >
              <span className="rounded-full" style={{ width: b * 360, height: b * 360, background: color }} />
            </button>
          ))}
        </div>
        <div className="flex gap-2">
          <button onClick={undo} className="pf-glass rounded-card px-4 py-2 text-sm">Undo</button>
          <button onClick={clear} className="pf-glass rounded-card px-4 py-2 text-sm">Clear</button>
        </div>
      </div>
    </div>
  )
