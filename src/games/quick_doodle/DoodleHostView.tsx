              ...
              {guesses.map((g) => {
                ...
                return (
                  <div key={g.id} className="flex items-center gap-2 text-sm">
                    {who && <span className="h-2.5 w-2.5 rounded-full" style={{ background: PAL[who.color] }} />}
                    <span className={g.is_correct ? 'font-bold text-p-lime' : 'text-white/70 line-through decoration-white/30'}>
                      {g.is_correct ? '✓ correct' : g.text}
                    </span>
                  </div>
                )
              })}
