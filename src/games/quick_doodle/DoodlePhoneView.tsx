...
import { ConfettiIcon, ShieldEyeIcon } from '../../shared/components/icons'
...
export default function DoodlePhoneView({ room, me }: PhoneViewProps) {
  ...
  useEffect(() => {
    ...
    // safety net against a missed broadcast on flaky wifi (race tier: 3s)
    const poll = window.setInterval(() => reload().catch(console.error), 3000)
    // instant resync when the phone returns to the foreground
    const onVisible = () => { if (document.visibilityState === 'visible') reload().catch(console.error) }
  ...
  async function send() {
    ...
    try {
      ...
      chan.current?.sendSync() // a correct guess ends the round; do not wait for a poll
  ...
  if (amDrawer) {
    ...
    return (
      <div className="flex h-[100dvh] max-h-[100dvh] flex-col overflow-hidden px-5 py-4">
        ...
        {round?.status === 'revealing' ? (
          <Center>
            <p className="pf-wordmark inline-flex items-center gap-2 text-2xl text-p-lime"><ShieldEyeIcon size={22} /> Shield your screen</p>
  ...
  return (
    <div className="flex min-h-full flex-1 flex-col px-5 py-6">
      ...
      {solvedByMe ? (
        <Center><span className="flex justify-center"><ConfettiIcon size={54} /></span><p className="pf-wordmark mt-2 text-4xl text-p-lime animate-popIn">You guessed it!</p></Center>
      ) : (
        <>
