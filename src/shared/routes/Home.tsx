...
export default function Home() {
  ...
  useEffect(() => { if (location.state) window.history.replaceState({}, '') }, [location.state])
  ...
  return (
    <Stage className="relative flex h-screen flex-col items-center overflow-hidden px-8 py-6">
      ...
      {notice && (
        <div className="pf-glass mb-4 flex items-center gap-4 rounded-card px-5 py-3 text-center text-white/85">
          <span>{notice}</span>
          <button onClick={() => setNotice(null)} className="text-sm text-cyan underline">Dismiss</button>
        </div>
      )}
      ...
      <footer className="mt-auto flex flex-wrap items-center justify-center gap-x-5 gap-y-1 pt-6 text-sm text-white/40">
        ...
        <a href="/blog/" className="hover:text-white/70">Blog</a>
