...
import { leaveRoom, closedReason, errorText } from '../lib/room'
...
type Phase = 'loading' | 'notfound' | 'failed' | 'join' | 'waiting' | 'playing'
...
export default function JoinShell() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      // Do not blame the room for something else going wrong.
      console.error('[Party Fever] join failed:', e)
      setJoinError(errorText(e))
      setPhase('failed')
    })
    return () => unsub?.()
  }, [code])
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(async () => {
      ...
      if (!r || r.status === 'closed') {
        ...
        navigate('/phone', {
          state: why === 'not_enough_players'
            ? { notice: 'The game closed because someone left and there were not enough players to carry on.' }
            : { notice: 'The host ended the game.' },
        })
        return
      }
      setRoom(r)
  ...
  if (phase === 'failed') {
    return (
      <Stage className="flex min-h-[100dvh] flex-col items-center justify-center px-8 text-center">
        <Wordmark size="sm" />
        <p className="mt-6 text-xl text-white/85">Could not join the game.</p>
        {joinError && <p className="mt-2 max-w-sm text-sm text-white/55">{joinError}</p>}
        <button
          onClick={() => window.location.reload()}
          className="mt-6 rounded-card px-6 py-3 font-bold text-grape-900"
          style={{ background: 'linear-gradient(95deg,#06b6d4,#84cc16)' }}
        >Try again</button>
        <button onClick={() => navigate('/phone')} className="mt-4 text-sm text-cyan underline">Back to home</button>
      </Stage>
    )
  }
  ...
  if (phase === 'playing' && room && me) {
    ...
    return (
      <Stage className="relative flex min-h-[100dvh] flex-col">
        {!finished && (
          <div className="absolute right-3 top-3 z-20">
            <button
              onClick={async () => {
                if (!window.confirm('Leave the game and return home?')) return
                try { await leaveRoom(room.id) } catch (e) { console.error(e) }
                navigate('/phone')
              }}
    ...
    return (
      <Stage className="relative flex min-h-[100dvh] flex-col items-center justify-center px-8 text-center">
        <button onClick={async () => {
          ...
          if (room) { try { await leaveRoom(room.id) } catch (e) { console.error(e) } }
          navigate('/phone')
        }}
  ...
  return (
    <Stage className="flex min-h-[100dvh] flex-col px-7 pb-10 pt-8">
      ...
      <button
        onClick={submit}
        disabled={!canSubmit}
        className="mt-8 rounded-card px-6 py-4 text-lg font-bold text-grape-900 transition disabled:opacity-40"
        style={{ background: 'linear-gradient(95deg,#06b6d4,#84cc16)' }}
      >
