...
import PhoneOutro from '../components/PhoneOutro'
...
export default function JoinShell() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      setMe(mine)
      setPhase(r.status !== 'lobby' && mine ? 'playing' : mine ? 'waiting' : 'join')
  ...
  if (phase === 'loading') {
    return <Stage className="flex min-h-screen items-center justify-center"><p className="text-white/70">Joining {code.toUpperCase()}…</p></Stage>
  }
  if (phase === 'notfound') {
    return (
  ...
  if (left) {
    return <Stage className="min-h-screen"><PhoneOutro title="See you next round!" /></Stage>
  }
  if (phase === 'playing' && room && me) {
    const mod = getModule(room.current_game)
    const finished = room.status === 'finished'
    return (
      <Stage className="relative min-h-screen">
        {!finished && (
          <div className="absolute right-3 top-3 z-20">
            <button
              onClick={() => window.confirm('Leave the game? You’ll drop out of the room.') && setLeft(true)}
              className="pf-glass rounded-full px-4 py-2 text-sm text-white/70"
            >Leave</button>
          </div>
        )}
        {mod ? <mod.PhoneView room={room} me={me} players={players} /> : <p className="p-6">Unknown game.</p>}
        {finished && <PhoneOutro title="Game over!" />}
      </Stage>
    )
  }
  if (phase === 'waiting' && me) {
    return (
      <Stage className="relative flex min-h-screen flex-col items-center justify-center px-8 text-center">
        <button onClick={() => window.confirm('Leave the room?') && setLeft(true)}
          className="pf-glass absolute left-3 top-3 rounded-full px-4 py-2 text-sm text-white/70">Leave</button>
