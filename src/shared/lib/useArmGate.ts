...
export function useArmGate(
  ...
  // Wake exactly when the window closes rather than polling for it. Polling
  // added up to a full tick of jitter between devices, on top of the clock
  // error, and it was the larger of the two.
  useEffect(() => {
    if (!active || !phaseStartedIso) return
    const target = new Date(phaseStartedIso).getTime() + ARM_MS
    const wait = target - serverNow()
    if (wait <= 0) return
    const id = window.setTimeout(() => tick((n) => n + 1), wait)
